...
class TouchTracking {
  ...
  initTouch(){
    ...
    $(document).on('pointerdown MSPointerDown', function (evnt) {
      if (_self.mode === _self.MOUSE_MODE || !_self.isMSTouchEvent(evnt)) return;
    ...
    $(document).on('pointermove MSPointerMove', function (evnt) {
      if (_self.mode !== _self.TOUCH_MODE || !_self.isMSTouchEvent(evnt)) return;
    ...
    $(document).on('touchstart', function (evnt) {
      if (_self.mode === _self.MOUSE_MODE) return;
    ...
    $(document).on('touchmove', function (evnt) {
      if (_self.mode !== _self.TOUCH_MODE) return;
      _self.dispatchEvent('dcg-tapmove', evnt);
    });
    ...
    ghostEvents.touchTrackingCallbacks.isGhostEvent = function (evt) {
      ...
      // legacy events.
      if (evt.target === _self.lastTarget) return false;
      if (evt.target && $.contains(evt.target, _self.lastTarget)) return false;
      if (_self.lastTarget && $.contains(_self.lastTarget, evt.target)) return false;
  ...
  filterSortedNodesWithinScope(sortedNodes) {
    var filtered = [];
  ...
  endMode(evnt) {
    ...
    if (
      this.modeEvents['dcg-tapstart'] === 1 &&
  ...
  dispatchEvent(type, evnt, differentTarget) {
    ...
    if (type === 'dcg-tapstart') {
      ...
      if (
        this.mode === this.TOUCH_MODE && lastEvnt && lastEvnt.type === 'dcg-tapstart' &&
  ...
  */
  ...
  var isTapActive(){
    return (this.mode !== this.NO_MODE);
  }
}


export let touchTracking = new TouchTracking();
